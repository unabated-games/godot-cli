...
pub fn exportCatalog(
    ...
    if (!dry_run) {
        const merged = try mergeIntoExisting(allocator, io, output_path, markdown);
        defer allocator.free(merged);
        io_util.writeFileAtomic(io, output_path, merged) catch return error.Io;
        wrote_file = true;
    }
...
pub const begin_marker = "<!-- godot-cli catalog: begin -->";
pub const end_marker = "<!-- godot-cli catalog: end -->";

/// The digest usually lives in a file that also holds hand-written rules
/// (AGENTS.md, CLAUDE.md). Replace only the digest: between the markers when
/// they exist, from the `# Component Catalog` heading to the end for a file
/// written before the markers, and appended after everything else otherwise.
fn mergeIntoExisting(allocator: std.mem.Allocator, io: std.Io, output_path: []const u8, digest: []const u8) ExportError![]u8 {
    const existing = std.Io.Dir.cwd().readFileAlloc(io, output_path, allocator, .unlimited) catch {
        return std.fmt.allocPrint(allocator, "{s}{s}\n", .{ digest, end_marker });
    };
    defer allocator.free(existing);

    const head_end: usize = blk: {
        if (std.mem.indexOf(u8, existing, begin_marker)) |at| break :blk at;
        if (std.mem.indexOf(u8, existing, "# Component Catalog")) |at| break :blk at;
        break :blk existing.len;
    };
    const tail_start: usize = blk: {
        if (std.mem.indexOf(u8, existing, end_marker)) |at| break :blk at + end_marker.len;
        break :blk existing.len;
    };

    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(allocator);
    const head = std.mem.trimEnd(u8, existing[0..head_end], "\n");
    if (head.len != 0) {
    ...
    const tail = std.mem.trim(u8, existing[@min(tail_start, existing.len)..], "\n");
    if (tail.len != 0) {
        try out.append(allocator, '\n');
