...
pub fn build(b: *std.Build) void {
    ...
    const version_string = b.option([]const u8, "version-string", "Version string embedded in the CLI") orelse "0.23.2";
    ...
    const version_date = b.option([]const u8, "version-date", "Release date (YYYY-MM-DD) of the embedded version") orelse "2026-09-10";
    ...
    const run_smoke = b.addSystemCommand(&.{
        ...
        \\out=$(./zig-out/bin/godot-cli project run --project-root test_fixtures/autoload_project --godot "$GODOT" --frames 6 --headless --click /root/Main/Play@2 --json || true) &&
        \\echo "$out" | grep -q '"error_count":0' &&
    ...
    const project_move_smoke = b.addSystemCommand(&.{
        ...
        \\printf '{ "ops": [ { "op": "assign_ext", "path": "/root/A", "property": "script", "type": "Script", "res_path": "res://scripts/p.gd", "id_hint": "p" } ] }' > "$tmp/p.json" &&
        ...
        \\./zig-out/bin/godot-cli project move --project-root "$tmp" --from scripts/hero/h.gd --to scripts/hero/hero.gd --rename-ids --json | grep -q '"ids_renamed":1' &&
        \\grep -q 'id="Script_hero"' "$tmp/scenes/a.tscn" &&
        \\grep -q 'script = ExtResource("Script_hero")' "$tmp/scenes/a.tscn" &&
        \\! grep -q 'Script_p"' "$tmp/scenes/a.tscn" &&
