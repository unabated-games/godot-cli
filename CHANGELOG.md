...
- **`project resave <file>`: Godot's own save of a scene or resource, as the reference for `scene compare-godot`.** It imports first, unless `--no-import`, then has Godot headless load the file and save a copy. The copy goes to `.godot/godot-cli/resave/<its path>` by default, never over the file itself, which it refuses as `resave_over_source`. Trial 35 wrote that GDScript by hand and ran Godot from the shell to get the same thing. A copy saved to a new path has its ext_resource ids renumbered and its uids left out. That is how Godot writes a save to a new path, and `compare-godot` allows for both.
- **`camera_3d` and `place_3d` recipes place and aim a 3D node.** `camera_3d` adds a `Camera3D` at `position` facing `look_at`, with optional `current` and `fov`. `place_3d` sets an existing 3D node's `transform` the same way. Each point is `[x, y, z]` or `"Vector3(x, y, z)"`. The basis is Godot's `looking_at` (-Z toward the point, +Y up), written row by row. Trial 35's camera, worked out by hand, comes out of `camera_3d` digit for digit, and a Godot re-save of the recipe's output matches it.
- **`uid read <file>`: the UID a file records, read from the file itself**, so it needs no uid cache and works on a fresh clone. It reads a scene or resource header, binary (`.res`, `.scn`) or text, a script's `.uid` sidecar, or an imported asset's `.import` file, and `source` says which. For a binary resource it also reports the class, whether the file is compressed, and the Godot version that saved it. A file that records no UID fails with `no_uid_recorded` and says why. Until now the only route to a binary resource's UID was `uid cache lookup`, which needs the project opened in Godot since the file last changed. Asked for by a user who needed a UID out of a binary resource and had no way to get one.
- **`project run` says when it ran a 3D scene with no camera**: `no_camera_3d: true`, and a message saying that unless a script adds a `Camera3D`, the frame shows only the clear colour, so the run proves the scene loads and not how it looks. It checks the scene and every scene it instances, and says nothing for a headless run or a scene it cannot read. Trials 31 to 34 each had to work out why their frame was grey. Adding a camera to trial 34's level made the note disappear and the meshes appear.
- **`scene compare-godot` compares UIDs, and says what differs.** A scene whose mesh uid had been swapped still reported `matches_godot_save: true`. The scene's uid and each ext_resource's uid are now compared wherever both files carry one. A reference saved from a script leaves them out, so a missing uid is not a difference. On a mismatch, `difference` names the first one. The descriptions now say what is compared and what is not (ext_resource ids, `load_steps`, sub_resources), and that the second positional and `--reference` are the same thing. A node's `instance` is now compared by the scene it names, not by its ext_resource id. Compared literally, every instanced node was a mismatch against a save to a new path, which renumbers those ids; `project resave`'s own test caught it.
- **`scene normalize` reports `changed`: whether the save changes the file.** So `--dry-run` answers "is this already written the way a save writes it?". A dry run also returns `preview_sections`, and says when a save would change only blank lines or section order. `scene apply --dry-run` now also runs the uid refresh a write does, so both previews show a write's exact text.
- **`scene validate` warns about a 3D node placed with `position`, `rotation`, `scale`, `quaternion`, `basis` or a `global_*` property (`property_not_stored`).** Godot loads such a line but never writes one, because a 3D node stores its placement as `transform` (`node_3d.cpp` gives those properties editor-only or no storage). So the editor's next save turns the line into `transform = Transform3D(...)`, a diff nobody asked for. The warning gives the `transform` form to use. Before shipping it was swept against Godot 4.8's own output: every instantiable `Node3D`, `Node2D` and `Control` class (218 of them), moved, rotated and scaled, then saved. Godot wrote `transform` for all 111 3D nodes and `position` for the 2D ones, and the check flagged nothing. No `Node3D` subclass in the class table declares its own property by any of those names.
- **`scene diff` reports resources**: ext_resources added, removed, or changed in type or uid, keyed by path because their local ids are renumbered freely; and sub_resources, keyed by id. With `--properties`, a sub_resource's changed properties are listed too, addressed as `SubResource("id")`. Added and removed nodes carry their `unique_id`, and a node whose `unique_id` changed at the same path is reported as `unique_id_changed`. Trial 32 had to find an added ext_resource through `scene describe`.
- **Every scene and resource write takes `--snapshot <path>`**: a copy of the file as it was before the write, for `scene diff` or `scene restore` afterwards. It is taken once per command, before anything is written, whether the command comes from the shell, a `batch` step, or MCP. A dry run takes none, and on `scene restore`, where `--snapshot` names the file to restore *from*, nothing is copied over it. `scene apply --auto-snapshot` now defaults to `.godot/godot-cli/snapshots/<the scene's path>` under the project root, where Godot imports nothing, instead of beside the scene. Trials 31 to 33 each kept a "before" copy by hand, and trial 32 left MCP to do it.
- **`scene apply --dry-run` returns `preview_sections`**: the exact text of every section a write would add or change, after the same uid refresh and save preparation a write does. Trial 33 needed a patch's text before writing it, and only `scene node add` could show one. On a patch that set a property and added a node, both previewed sections appear verbatim in the file the real write produced.
- **`scene node add --dry-run` and `scene instance add --dry-run` return `section_text`**: the section a write would put in the file, after the same preparation a write does, `unique_id` included. Trial 32 dry-ran an add to see how a property would be written and got no text back. The descriptions say the preview's `unique_id` is the one a write assigns, being seeded from the scene's path. Trial 33 could not tell that, and worked around it with a patch op that pins the id.

### Fixed

- **Saving a scene rewrote correct UIDs on its references to binary resources (`.res`, `.scn`) whenever the project had no `.godot/uid_cache.bin`, which is every fresh clone, since `.godot/` is gitignored.** Any write with `--project-root` did it. In the reproduction, a `set-property` on an unrelated node replaced `uid://bc628hhe4x5yp` with `uid://byggqned6p7ih` and exited 0. Godot then warned `invalid UID … using text path instead` on every load, and the reference no longer survived the file being moved, which is what the UID is for. A binary resource keeps its UID inside its own header, and godot-cli read headers only from `.tscn` and `.tres` files. For anything else it computed a UID from the file's bytes, and those bytes contain the UID, so the hash can never equal it. `scene ext add` of a `.res` with no cache wrote a wrong UID the same way. godot-cli reads the binary header now, including the compressed form the editor saves by default, and a save repairs references the old behaviour damaged. Found while looking into the request above.
- **Rewriting a scene put a blank line between consecutive `[ext_resource]` lines, so even a no-op `scene normalize` of a file Godot saved was not byte-identical.** Godot writes that block as adjacent lines and puts one blank line after it (`ResourceFormatSaverTextInstance::save`), so every scene with two or more ext_resources got a diff on lines nobody had touched, the first time godot-cli wrote it. No Godot-saved fixture had more than one ext_resource, which is how this went unnoticed. A new one, `multi_ext_godot_saved.tscn`, saved by Godot 4.8, has three. Files godot-cli wrote before lose those blank lines on their next edit, as they would on Godot's next save.
- **An added reference put `uid` after `id`, and an instanced node given a `unique_id` put it after `instance=`.** Godot writes `type, uid, path, id` on an ext_resource and ends a node line with `instance`. Both now land where the editor writes them.
- **`scene diff --properties` said nothing about an added node's properties, and gave an instanced node an empty type.** Trial 31 added a mesh with a `transform` and a `mesh`, got `property_diff_count: 0`, and fell back to `diff -u` for its answer. An added node now lists the properties it arrived with, and a removed node the properties it took with it. An instanced node reports its scene's root class when there is a project root, as `scene describe` does, and `PackedScene` when there isn't; `instance_path_a` or `instance_path_b` names the scene. `scene apply --dry-run --preview-properties` shows an added node's properties the same way.
- **`scene node get` on a missing node failed as `command_failed` with the message "Usage".** It now fails as `node_not_found`, naming the argument and the path. It gives the same hint `scene set-property` gives, including the one for a path inside an instanced scene. The two commands now share that code.
- **A float property given a whole number, or a JSON float that happened to be whole, was written without `.0`.** `{"light_energy": 2.0}` came out as `light_energy = 2`, where Godot writes `2.0`. That held through `scene node add --properties`, `scene set-property`, patches and intents. There were two causes. `{d}` prints 2.0 as `2`. And whole numbers were judged by a hand-written list of name prefixes, which matched `energy` but not `light_energy`. The class table now decides wherever the class is known: a float property keeps its fractional part and an int property drops it (`render_priority = 2`). The name list stays only as the fallback for classes the table lacks.
- **The tests against committed Godot saves had never run.** Nothing the test root reaches imported `src/godot/fixtures.zig`, or one test in `src/cli/json_input.zig`, so five tests had never run: three against the committed fixtures, one on JSON requests, and the block that pulls in the rest. `fixtures.zig` did not even compile any more. All 247 tests run now, where 241 did, on every platform, the Windows job included. The five older ones pass.
- **`scene validate` reported correct references to binary resources as `stale_uid_for_path`**, from the same hash. It did the same for a reference to an imported asset after the asset was edited and reimported: Godot keeps the UID in the `.import` file when the bytes change, and validate hashed the new bytes instead of reading it. Its suggested fix, `scene normalize`, could not clear the warning. Both cases now compare against the UID Godot recorded.

### Changed

One change alters JSON output that scripts may read: the UID numbers below are now strings.

- **A UID's number is a decimal string in JSON**: `uid decode`'s `data`, `uid create-for-path`'s `id`, and `uid cache list`'s `id` on every entry. A 64-bit number does not survive a JSON parser that reads numbers as doubles. JavaScript's parser, which Claude Code's MCP client uses, handed trial 30 `5402782583183944000` for `5402782583183943612`. `uid decode` also now prints that number instead of echoing its input back. And `uid create-for-path`'s summary now says what it is, the UID Godot would assign a *new* file, and points at `uid read`: on a binary resource it gives a plausible number that is wrong, and trial 31 named it as the easiest tool to misuse.
- **A missing uid cache fails as `uid_cache_missing`, not `io`.** "Could not read or write a file" read as a filesystem fault, on every fresh clone. The hint now names `project import` and `uid read`, and says what `scene validate` still checks without a cache. An unreadable cache is still `uid_cache_unreadable`.
- **The agent rules say how to instance a scene that has no catalog entry: by its path, with `--scene res://...`.** They showed only `--catalog-id`, and trial 31, whose project had no manifests, had to work the path form out from the tool schema. The fallback is now in the rules block in `how-to/agent-setup.md`, the component guide, the quickstart, the skill, and the MCP session prompt, which all state the same rule.
- **`project import` and `project run` say that the import writes into the project**: a `.import` file beside each asset, a `.uid` file beside each script, and `.godot/`. Trial 31 had been told to change nothing unexpected, and checksummed the whole project to prove what the import had touched. `project import` also says to commit those files, and why.
- **Descriptions corrected after trial 33.** `project run` said it captured "into capture/", but the default is under `.godot/`. The authoring doc called `--project-root` "ignored" on `scene node list` and `scene diff`, but both use it to resolve instanced nodes. `--properties` now says it wins when it and `--property`/`--value` set the same property. `scene diff` documents `instance_of`. The rules block now also says `--frames 5` is enough for a screen that does not move. After trial 34: `--snapshot` says why the copy belongs under `.godot/` (Godot reports a duplicate uid for a copy it can see, and may point `uid://` references at it); `preview_sections` says it comes in file order, with the unique_ids a write assigns; and the authoring doc shows how to place a 3D node with `transform`, basis row by row as Godot writes it. After trial 36: `project resave` says its check is semantic, not byte for byte, and why; and the rules name the `camera_3d` and `place_3d` recipes, and say that with no camera none of the scene appears. The rules' MCP note no longer says options lose "the dashes": only the leading `--` goes, and `--dry-run` is `dry-run`.
- **A pinned MCP server's refusal of a path outside the project says where a scratch copy can go**: under `.godot/`, which Godot does not import and which is normally gitignored. Trials 31 and 32 each kept a "before" copy of a scene to diff against. Trial 32 put its copy in `/tmp`, was refused with no way forward, and ran the godot-cli binary from the shell instead.
- **The agent rules say how they map onto MCP**, which has no `--json` or `--project-root` to pass. They also say that `project run`'s import writes `.import` and `.uid` files, which belong in version control, and that a 3D scene with no `Camera3D` renders a plain grey frame, so such a run proves only that the scene loads.
- **MCP tool descriptions name fields, not command-line flags.** Over MCP an option is a field: `--parent` now reads `` `parent` ``, a pinned server's `--project-root` reads "the server's project root", and "(with --json)" is gone, since MCP output is always JSON. That changes 86 of the 95 flag mentions an agent saw. The rest quote Godot's own command line, such as `godot --headless --path . --import --quit`, and stay as written. Every `properties` description also gains a constructor example (`"position": "Vector2(3, 0)"`, `"mesh": "ExtResource(\"2_rock\")"`). It uses a 2D position because that is what Godot stores: a 3D node stores `transform`, and `position` is never written for it, which trial 32 caught in a first draft that used `Vector3`.
