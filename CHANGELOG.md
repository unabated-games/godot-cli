...
- **`stale_uid_for_path` compared scene and resource references against a hash of the file**, which a header uid never matches; it compares against the header now. New `ext_resource` sections gain a `uid=` when the target has one recorded (header, `.uid` sidecar, or `.import`), as the editor writes; a guess is never added.
